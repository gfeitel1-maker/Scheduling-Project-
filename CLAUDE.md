...
**Styling** — global design tokens live in CSS: `src/index.css` defines `--primary` and the whole token set (`src/App.css` holds app-shell rules). Component styles are inline React objects; shared constants live in `src/styles/shared.js`, imported as `import { S } from '../styles/shared'`, with component-specific styles as `const` objects at the bottom of each file. No CSS modules.
