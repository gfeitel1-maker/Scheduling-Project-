...
export default function SlotCell({ slot, activity, anchor, actColorIdx, weatherMode, onEdit, onLock, onRelease, isLocked, isDndEnabled }) {
  ...
  if (slot.type === 'anchor') {
    return (
      <td ref={setRef} style={cellTd} onClick={() => onEdit(slot)}>
        <div style={{ background: '#F3E8FA', border: '1.5px solid #A6359566', borderRadius: 8, padding: '10px 12px', minHeight: 56, display: 'flex', alignItems: 'center' }}>
  ...
  const lockedInnerStyle = { background: '#FFFBF0', border: '2px solid #E8A020', borderRadius: 8, padding: '10px 12px', minHeight: 56, position: 'relative', overflow: 'hidden' }

  const normalInnerStyle = activity
    ? { background: `${color}1E`, border: isWeatherHighlight ? `2px solid #2F7DE1` : `1.5px solid ${color}55`, borderRadius: 8, padding: '10px 12px', minHeight: 56, opacity: isDragging ? 0.4 : 1, outline: isOver && isDndEnabled ? '2px solid var(--primary)' : 'none', outlineOffset: -2, position: 'relative' }
    : { background: 'var(--bg)', border: '1.5px dashed #D8C8B8', borderRadius: 8, padding: '10px 12px', minHeight: 56, position: 'relative' }
  ...
  return (
    <td ref={setRef} style={{ ...cellTd, cursor: canDrag ? (isDragging ? 'grabbing' : 'grab') : 'pointer' }} onClick={handleClick} onContextMenu={handleContextMenu} title={tooltipText} {...(canDrag ? { ...listeners, ...attributes } : {})}>
      <div style={innerStyle}>
        {isLocked && (
          <div style={{ position: 'absolute', top: 0, right: 0, width: 0, height: 0, borderTop: '12px solid #E8A020', borderLeft: '12px solid transparent' }} />
        )}
        <div style={{ fontSize: 12, fontWeight: activity ? 700 : 500, color: isLocked ? '#7A5100' : (activity ? color : '#B0A090'), overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {activity?.name || <span style={{ fontSize: 11 }}>Unassigned</span>}
        </div>
        {hasFlags && !isLocked && (
          <div style={{ display: 'flex', gap: 2, marginTop: 4, flexWrap: 'wrap' }}>
            {activeFlags.map(f => (
              <span key={f} style={{ width: 6, height: 6, borderRadius: '50%', background: FLAG_COLORS[f], display: 'inline-block' }} title={flags[`${f}_reason`] || f} />
            ))}
          </div>
        )}
      </div>
