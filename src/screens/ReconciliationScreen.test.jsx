...
// is a distinct screen now (RootsHomeScreen); ReconciliationScreen keeps
// mode="import" as its only caller going forward.
describe('import-only screen (mode="inspect" retired)', () => {
  it('renders the import flow with no mode prop needed (ImportScreen call site unchanged)', async () => {
    localClient.ingestReconcile.mockResolvedValue(understoodOnlyResult())
    render(<ReconciliationScreen baseInputs={baseInputs} sourceLabel="camp.xlsx" onCommitted={vi.fn()} onDiscard={vi.fn()} onNavigate={vi.fn()} />)
