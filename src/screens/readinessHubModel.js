...
export function buildHubRows(readiness, counts = {}) {
  ...
  const required = [
    ...
    row(by.activities, 'Activities', countFor('activities'), 'two'),
    activityRulesRow(by.activities),
  ]
...
// Activity Rules mirrors Activities: derived, never blocking, two doors (its
// worksheet is embedded in the Activities sheet). With no live reconciliation
// session there is no at-rest "inferred, not confirmed" signal to read, so it
// degrades to Ready when activities exist.
