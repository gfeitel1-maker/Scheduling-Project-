...
describe('TiersScreen — add', () => {
  it('adds a tier by writing each field via localClient.write, cohort_id then name', async () => {
    localClient.list.mockReset().mockImplementation(entity => {
      if (entity === 'cohorts') return Promise.resolve([cohort()])
      if (entity === 'tiers') return Promise.resolve([])
      if (entity === 'groups') return Promise.resolve([])
    ...
    // T238: tiers is UNIQUE(camp_id, cohort_id, name), so detectUniqueFieldCollision
    // scopes its check by cohort_id as well as camp_id — and it reads that scope value
    // off the row that already exists. cohort_id must therefore be written BEFORE name,
    // or the check has no cohort to scope by and skips rather than guessing. This test
    // used to assert `name` was written first, which was correct while the check was
    // camp-scoped only; the ordering below is the contract that replaced it.
    expect(fieldsWritten.indexOf('cohort_id')).toBeLessThan(fieldsWritten.indexOf('name'))
    // `name` still precedes camp_id — the unique field goes early so the collision
    // check fires on a create rather than after unrelated fields have landed.
