...
// badge; a false positive would tell a director they are looking at a week they
// are not.
...
export function snapshotMatchesSchedule(snapshot, { slots } = {}) {
  const parsed = parseSnapshotPayload(snapshot)
  if (!parsed.ok) return false
  return canonical(parsed.slots) === canonical(slots)
}
