...
describe('LocationsScreen — week availability (M5)', () => {
  ...
  // ── v50 indoor/outdoor map pair (Slice 2) ──────────────────────────────────
  const GEO = (x, y) => JSON.stringify({ x, y, w: 0.08, h: 0.08 })
  function seedMaps(campMaps, locations) {
    localClient.list.mockImplementation((entity) => {
      if (entity === 'camp_maps') return Promise.resolve(campMaps)
      if (entity === 'locations') return Promise.resolve(locations)
      return Promise.resolve([])
    })
  }

  it('single-map camp shows NO Buildings/Grounds toggle on the Map tab', async () => {
    seedMaps(
      [{ id: CAMP_ID, camp_id: CAMP_ID, kind: null, image_data: 'img', image_mime: 'image/png', image_width: 100, image_height: 100 }],
      [location({ id: 'l-1', name: 'Pool', map_geometry: GEO(0.1, 0.1) })],
    )
    render(<LocationsScreen campId={CAMP_ID} role="admin" onNavigate={() => {}} />)
    await waitFor(() => expect(screen.queryByText('Pool')).not.toBeNull())
    fireEvent.click(screen.getByText('Map'))
    await waitFor(() => expect(screen.queryByText('Pool')).not.toBeNull()) // marker rendered
    expect(screen.queryByRole('tab', { name: /Buildings/ })).toBeNull()
    expect(screen.queryByRole('tab', { name: /Grounds/ })).toBeNull()
  })

  it('two-map camp shows the toggle and scopes markers to the active map', async () => {
    seedMaps(
      [
        { id: CAMP_ID, camp_id: CAMP_ID, kind: 'indoor', image_data: 'a', image_mime: 'image/png', image_width: 100, image_height: 100 },
        { id: 'map-out', camp_id: CAMP_ID, kind: 'outdoor', image_data: 'b', image_mime: 'image/png', image_width: 100, image_height: 100 },
      ],
      [
        // map_id null → resolves to the primary (indoor) map
        location({ id: 'l-in', name: 'Room 1', map_geometry: GEO(0.1, 0.1), map_id: null }),
        location({ id: 'l-out', name: 'Soccer Field', map_geometry: GEO(0.3, 0.3), map_id: 'map-out' }),
      ],
    )
    render(<LocationsScreen campId={CAMP_ID} role="admin" onNavigate={() => {}} />)
    await waitFor(() => expect(screen.queryByText('Room 1')).not.toBeNull())
    fireEvent.click(screen.getByText('Map'))

    // Toggle present; Buildings active by default → indoor marker shown, outdoor hidden.
    await waitFor(() => expect(screen.queryByRole('tab', { name: /Buildings/ })).not.toBeNull())
    expect(screen.queryByRole('tab', { name: /Grounds/ })).not.toBeNull()
    expect(screen.queryByText('Room 1')).not.toBeNull()
    expect(screen.queryByText('Soccer Field')).toBeNull()

    // Switch to Grounds → outdoor marker shown, indoor hidden.
    fireEvent.click(screen.getByRole('tab', { name: /Grounds/ }))
    await waitFor(() => expect(screen.queryByText('Soccer Field')).not.toBeNull())
