    ...
    return (
      <tr style={{ background: 'var(--surface-elevated)' }}>
        ...
        <td style={{ ...S.td, textAlign: 'right' }}>
          ...
          <button onClick={() => { setName(tier.name); setSortOrder(tier.sort_order); setEditing(false) }} style={{ ...S.btnSecondary, marginLeft: 6 }}>Cancel</button>
        </td>
      </tr>
    )
  }
  ...
  return (
    <tr style={{ borderBottom: '1px solid var(--border)' }} onMouseEnter={e => e.currentTarget.style.background = 'var(--bg)'} onMouseLeave={e => e.currentTarget.style.background = ''}>
      ...
      <td style={{ ...S.td, textAlign: 'right' }}>
        ...
        <button onClick={() => onDelete(tier.id)} style={{ ...S.btnDanger, marginLeft: 6 }} disabled={groupCount > 0} title={groupCount > 0 ? 'Remove groups from this tier first' : ''}>Delete</button>
      </td>
    </tr>
  )
}
...
export default function TiersScreen({ campId, onNavigate }) {
  ...
  async function load() {
    ...
    try {
      ...
      setTiers(tierData || [])
      const counts = {}
      for (const g of groupData || []) { counts[g.tier_id] = (counts[g.tier_id] || 0) + 1 }
  ...
  async function addTier() {
    ...
    setNewName(''); setNewSort(''); setAdding(false); load()
  }

  async function saveTier(id, name, sort_order) {
    await supabase.from('tiers').update({ name, sort_order }).eq('id', id); load()
  }

  async function deleteTier(id) {
    if (!window.confirm('Delete this tier?')) return
    await supabase.from('tiers').delete().eq('id', id); load()
  }

  async function deleteAll() {
    if (!window.confirm('Delete all tiers? This cannot be undone.')) return
    await supabase.from('tiers').delete().eq('camp_id', campId); load()
  }

  function downloadTemplate() {
    const ws = XLSX.utils.aoa_to_sheet([['name', 'sort_order'], ['Yeladim', 1]])
  ...
  function onFileChange(e) {
    ...
    reader.onload = ev => {
      ...
      const parsed = rows.map(r => ({ name: String(r.name || '').trim(), sort_order: r.sort_order !== '' ? Number(r.sort_order) : null, warning: !String(r.name || '').trim() ? 'Missing name' : null }))
  ...
  return (
    <div style={{ maxWidth: 700 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div style={{ fontFamily: 'var(--font-condensed)', fontWeight: 700, fontSize: 13, color: 'var(--text-secondary)', letterSpacing: '0.05em', textTransform: 'uppercase' }}>{tiers.length} tier{tiers.length !== 1 ? 's' : ''}</div>
      ...
      {loading ? <div style={{ color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)', fontSize: 13 }}>Loading…</div> : (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden', marginBottom: 16 }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr style={{ borderBottom: '1.5px solid var(--border)', background: 'var(--surface-elevated)' }}><th style={S.th}>Name</th><th style={S.th}>Sort Order</th><th style={S.th}>Groups</th><th style={{ ...S.th, textAlign: 'right' }}>Actions</th></tr></thead>
            <tbody>
              {tiers.length === 0 ? (<tr><td colSpan={4} style={{ padding: '40px 16px', textAlign: 'center' }}><div style={{ fontFamily: 'var(--font-condensed)', fontSize: 16, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 4 }}>No tiers yet</div><div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Add your first tier below or import from Excel.</div></td></tr>) : tiers.map(tier => (<TierRow key={tier.id} tier={tier} groupCount={groupCounts[tier.id] || 0} onSave={saveTier} onDelete={deleteTier} />))}
            </tbody>
          </table>
        </div>
      )}
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '16px 18px' }}>
        <div style={{ fontFamily: 'var(--font-condensed)', fontWeight: 700, fontSize: 13, marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Add Tier</div>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <input placeholder="Tier name (e.g. Yeladim)" value={newName} onChange={e => setNewName(e.target.value)} onKeyDown={e => e.key === 'Enter' && addTier()} style={{ ...S.input, flex: 1 }} />
          <input type="number" placeholder="Order" value={newSort} onChange={e => setNewSort(e.target.value)} style={{ ...S.input, width: 80 }} />
          <button onClick={addTier} disabled={adding || !newName.trim()} style={S.btnPrimary}>{adding ? 'Adding…' : '+ Add'}</button>
        </div>
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'var(--surface-elevated)', borderRadius: 12, padding: 28, width: 520, maxHeight: '80vh', overflow: 'auto' }}>
            {importStep === 'preview' && (<>
              ...
              <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 16 }}>{readyRows.length} row{readyRows.length !== 1 ? 's' : ''} ready{warnRows.length > 0 && `, ${warnRows.length} with warnings (will be skipped)`}</div>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 18 }}><thead><tr style={{ borderBottom: '1px solid var(--border)' }}><th style={S.th}>Name</th><th style={S.th}>Sort Order</th><th style={S.th}>Status</th></tr></thead><tbody>{importRows.map((r, i) => (<tr key={i} style={{ background: r.warning ? '#FFF8E7' : '', borderBottom: '1px solid var(--border)' }}><td style={S.td}>{r.name || <span style={{ color: 'var(--warning)' }}>—</span>}</td><td style={{ ...S.td, fontFamily: 'var(--font-mono)', fontSize: 12 }}>{r.sort_order ?? '—'}</td><td style={{ ...S.td, color: r.warning ? '#F5A623' : 'var(--success)', fontFamily: 'var(--font-mono)', fontSize: 12 }}>{r.warning || '✓ Ready'}</td></tr>))}</tbody></table>
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                <button onClick={() => { setImportStep(null); setImportRows([]) }} style={S.btnSecondary}>Cancel</button>
                <button onClick={confirmImport} disabled={importing || readyRows.length === 0} style={S.btnPrimary}>{importing ? 'Importing…' : `Import ${readyRows.length} tier${readyRows.length !== 1 ? 's' : ''}`}</button>
              </div>
            </>)}
            {importStep === 'done' && (<>
              <div style={{ fontFamily: 'var(--font-condensed)', fontWeight: 700, fontSize: 17, marginBottom: 12 }}>Import Complete</div>
              <div style={{ fontSize: 14, marginBottom: 6 }}><span style={{ color: 'var(--success)', fontWeight: 600 }}>{importResult.added} added</span>{importResult.skipped > 0 && <span style={{ color: 'var(--text-secondary)', marginLeft: 10 }}>{importResult.skipped} skipped</span>}</div>
