...
function TierRow({ tier, groupCount, onSave, onDelete }) {
  ...
  if (editing) {
    return (
      <tr style={{ background: 'var(--surface-elevated)' }}>
        ...
        <td style={td}>{groupCount}</td>
        <td style={{ ...td, textAlign: 'right' }}>
          <button onClick={save} disabled={saving} style={btnPrimary}>
            {saving ? 'Saving…' : 'Save'}
          </button>
          <button onClick={() => { setName(tier.name); setSortOrder(tier.sort_order); setEditing(false) }} style={{ ...btnSecondary, marginLeft: 6 }}>
            Cancel
          </button>
        </td>
      </tr>
  ...
  return (
    ...
    >
      <td style={td}>{tier.name}</td>
      <td style={{ ...td, fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-secondary)' }}>{tier.sort_order}</td>
      <td style={{ ...td, fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-secondary)' }}>{groupCount}</td>
      <td style={{ ...td, textAlign: 'right' }}>
        <button onClick={() => setEditing(true)} style={btnSecondary}>Edit</button>
        <button onClick={() => onDelete(tier.id)} style={{ ...btnDanger, marginLeft: 6 }}
    ...
    <div style={{ maxWidth: 700 }}>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '14px 16px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          />
          <button onClick={addTier} disabled={adding || !newName.trim()} style={btnPrimary}>
            {adding ? 'Adding…' : '+ Add'}
          </button>
        </div>
      </div>
            ...
            {importStep === 'preview' && (
              <>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 18 }}>
                  <thead>
                    <tr style={{ borderBottom: '1px solid var(--border)' }}>
                      <th style={th}>Name</th><th style={th}>Sort Order</th><th style={th}>Status</th>
                    </tr>
                  </thead>
                  <tbody>
                    {importRows.map((r, i) => (
                      <tr key={i} style={{ background: r.warning ? '#FFF8E7' : '', borderBottom: '1px solid var(--border)' }}>
                        ...
                        <td style={{ ...td, color: r.warning ? '#F5A623' : 'var(--success)', fontFamily: 'var(--font-mono)', fontSize: 12 }}>
                          {r.warning || '✓ Ready'}
                        </td>
                      </tr>
                    ))}
                ...
                <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                  ...
                  <button onClick={confirmImport} disabled={importing || readyRows.length === 0} style={btnPrimary}>
      ...
      <div style={{ marginTop: 28, paddingTop: 20, borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'flex-end' }}>
        <button onClick={() => onNavigate('groups')} style={btnPrimary}>Next: Groups →</button>
      </div>
    </div>
  )
}
