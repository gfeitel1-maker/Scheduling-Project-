...
| **`projection_failures`** | **A** only | `projectionRepair.js` (`store='projection'`), `documentWriteFailures.js` (`store='document'`) | Never syncs — this device's own diagnostics | `listDocumentWriteFailures(db)`; the `store` column is what keeps the two repairs from crossing |
