...
_Last updated: 2026-08-09 (reconciliation program closed: host-local source aliases + "Remember this", fixed-event routing out of the activity catalog, reviewable units, and re-import provenance protection for units/groups/activity-rules)_
...
| `readiness` | `src/screens/ReadinessHub.jsx` | Default/landing screen inside a session (S5, six-state readiness model) — reports whether the camp can build a week yet |
| `camp` | `src/screens/CampScreen.jsx` | Camp-level setup entity screen |
| `import` | `src/screens/ImportScreen.jsx` | Onboarding/reconciliation: ingest a prior-year schedule or workbook, surface recognition matches and held conflicts. Reviews now include: an editable **unit column** per group (assign/confirm tiers the file can't encode), **fixed-event routing** (a pinned event like Lunch defaults out of the activity catalog with a one-click keep-it override), and **"Remember this"** on identity-conflict cards (persists a confirmed alias). Director hand-edits (units, groups, activity rules) are stamped human provenance so a re-import can't silently overwrite them. |
...
(SQLite, one file per device — `electron/db/schema.sql`, schema v31 as of this writing)
...
Not role-differentiated at the top level — `readiness` (`src/screens/ReadinessHub.jsx`) is the fixed landing screen for any session regardless of `admin`/`staff` role (`App.jsx:68`, S5: the six-state readiness model). No per-role home screen variants found. The setup entity screen itself is `camp` (`src/screens/CampScreen.jsx`), reachable from the sidebar, not the landing screen.
