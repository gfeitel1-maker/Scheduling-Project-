...
1. Restoring an activity states that the cells it was removed from are still empty.
2. Restoring a group states that its week did not come back, and points to Versions by name.
3. The wording is a director's, not a developer's — no "snapshot", no "op", no "slot".
4. A test asserts the caveat is present on both restore paths.
