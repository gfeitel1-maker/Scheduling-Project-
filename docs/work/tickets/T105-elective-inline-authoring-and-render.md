...
depends_on: [docs/work/tickets/T111-elective-cell-atomic-content-and-mutual-exclusion.md]
...
interaction), on **both routes**. The management screen (T110) is the secondary surface.

## Scope

- Extend `CellInlineEditor`/`SlotCell` so typing an elective name + members writes the set + members +
  the cell's elective content in one gesture (uses T111's atomic write).
...
## Binding constraints from T110 Red Hat review (MEDIUM — must honor here)

T110 shipped the durability data layer but has **no consuming read path yet**. This ticket wires reads,
so it inherits two guarantees Red Hat flagged (2026-08-20, T110 review):
...
## Inherited obligations from T111 design + Red Hat (must design/build here)

3. **Span-head → elective conversion must be a multi-cell atomic write.** T111's apply-time invariant is
...
4. **`CONTENT_RACE` flag is only mechanism-level in the T111 design — T105 must fully specify it.** The
   ...
   replaced by a concurrent edit." T111 makes the guarantee true; T105 designs + builds the surface.
