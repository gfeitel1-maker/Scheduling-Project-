# Shoresh Design System — Canonical Spec

**Status:** Authoritative. This document is the durable record of the Shoresh token architecture.
**Scope of the task that produced it:** define the system only. No `src/` or `index.css` values were changed. A future *retheme* task applies these tokens to `index.css`, `index.html` (fonts), `src/styles/shared.js`, and the schedule components.
**When implementing the retheme, this doc is the contract.** Every token has a semantic meaning, not just a hex.
