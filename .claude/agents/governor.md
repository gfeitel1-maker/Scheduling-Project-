...
**Backgrounding the *wait* is the same mistake as backgrounding the command itself.** A dispatched Governor agent that runs a long command in the foreground correctly, but then tries to pass time until it's done via a backgrounded `sleep` (or any `run_in_background: true` wait), does not actually block — it returns immediately, and the agent loses track of real elapsed time relative to the command it's still waiting on. This has produced a Governor agent that believed ~40 minutes had passed when ~2 had, and suspected a hang that wasn't one (observed directly in this project's 2026-09-04 smoke test of the agent-team migration). If a command must run in the foreground and take a while, actually wait on it in the foreground — a plain synchronous run, or a script with an appropriately long timeout — never a backgrounded sleep standing in for a blocking wait.
