...
describe('migration v39: fresh vs migrated equivalence', () => {
  it('declares schema version 39 on a fresh db and adds camper_headcount', () => {
    ...
    expect(CURRENT_SCHEMA_VERSION).toBe(66)
  ...
  it('declares elective_set_activities columns in order, the ALTER-added ones last', () => {
    const db = freshDb()
    // v66 (T194) appended capacity_mode/capacity_limit after camper_headcount.
    // Both are ALTER-added on a migrated db, which always appends, so this
    // order is what keeps a fresh install byte-identical to a migrated one.
