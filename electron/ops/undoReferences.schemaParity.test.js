...
const ACCEPTED_NON_REFERENCES = [
  ...
  { table: 'sync_health_events', column: 'camp_id', reason: 'scopes to camps, not a U2-deletable entity (host-local device-health diagnostics, T174 — never undone, never replicated)' },
  ...
  { table: 'import_decisions', column: 'learned_from_id', reason: 'points at a future camp_seedlings row (unused/NULL until T173 slice 3), not a U2-deletable entity' },
  { table: 'import_decision_failures', column: 'camp_id', reason: 'scopes to camps, not a U2-deletable entity (host-local decision-journal-write-failure trace, T173 slice 1 — never undone, never replicated)' },
